...
namespace codex::gfx {
    ...
    void DebugDraw::Render()
    {
        ...
        for (const auto& line : m_Lines)
        {
            ...
            for (i32 i = 0; i < 2; ++i)
            {
                ...
                // std::memcpy(m_Verticies.data() + index + sizeof(3 * sizeof(f32)), ValuePtr(colour), sizeof(4 *
                // sizeof(f32)));
            ...
            // m_Ebo->Bind();
            m_Instance->m_Vbo->Bind();
            m_Instance->m_Vbo->SetBufferSubData<f32>(m_Verticies.data(), 0,
                                                     LINE2D_VERTEX_SIZE * m_Lines.size() * sizeof(f32));
    ...
    void DebugDraw::DrawRect2D(const Rectf rect, const f32 angle, const Vector4f colour, const i32 lifeTime)
    {
        if (m_Lines.size() + 4 < LINE2D_MAX_LINES)
        {
            ...
            m_Lines.emplace_back(glm::vec2{ lines[0].x, lines[0].y }, glm::vec2{ lines[1].x, lines[1].y }, colour,
                                 lifeTime);
            m_Lines.emplace_back(glm::vec2{ lines[0].x, lines[0].y }, glm::vec2{ lines[3].x, lines[3].y }, colour,
                                 lifeTime);
            m_Lines.emplace_back(glm::vec2{ lines[1].x, lines[1].y }, glm::vec2{ lines[2].x, lines[2].y }, colour,
                                 lifeTime);
            m_Lines.emplace_back(glm::vec2{ lines[2].x, lines[2].y }, glm::vec2{ lines[3].x, lines[3].y }, colour,
                                 lifeTime);
        }
    }

    void DebugDraw::DrawCircle2D(const Vector2f centrePos, const i32 radius, const i32 segments, const Vector4f colour,
                                 const i32 lifeTime)
    {
