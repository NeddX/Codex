...
namespace codex {
    ...
    void Window::Init(const WindowProperties& windowInfo,
                      const void*             nativeWindow)
    {
        ...
        if (m_NativeWindow)
        {
            ...
            m_SdlWindow = SDL_CreateWindow(
                m_Title.c_str(), m_PosX, m_PosY, m_Width, m_Height,
    ...
    void Window::SDLThrowError(const i32              line,
                               const std::string_view errorMessage)
    {
        fmt::println("SDL ERROR @ LINE {}: {} -> {}", line, errorMessage,
                     SDL_GetError());
        SDL_Quit();
        exit(-1);
    }
        ...
        while (SDL_PollEvent(&m_SdlEvent))
        {
            ...
            switch (m_SdlEvent.type)
            {
                ...
                case SDL_MOUSEBUTTONDOWN: {
                    MouseDownEvent e(Mouse(m_SdlEvent.button.button),
                                     m_SdlEvent.motion.x, m_SdlEvent.motion.y);
                    if (m_EventCallback)
                    {
                        m_EventCallback(e);
                    }
                ...
                case SDL_MOUSEBUTTONUP: {
                    MouseUpEvent e(Mouse(m_SdlEvent.button.button),
                                   m_SdlEvent.motion.x, m_SdlEvent.motion.y);
                    if (m_EventCallback)
                    {
                        m_EventCallback(e);
                ...
                case SDL_MOUSEWHEEL: {
                    MouseScrollEvent e(Mouse(m_SdlEvent.button.button - 1),
                ...
                case SDL_KEYDOWN: {
                    KeyDownEvent e(Key(m_SdlEvent.key.keysym.sym),
                                   m_SdlEvent.key.repeat);
                    if (m_EventCallback)
                    {
                        m_EventCallback(e);
                    }
