	...
	namespace {
		...
		namespace Internals {
			...
			void AttachDepthTexture(uint32_t id, const TextureProperties& props, uint32_t width, uint32_t height)
			{
				...
				GL_Call(glFramebufferTexture2D(GL_FRAMEBUFFER, GetDepthAttachmentTypeFromFormat(props.format), GL_TEXTURE_2D, id, 0));
			}
		}
	}
	...
	void FrameBuffer::Invalidate()
	{
		...
		if (!m_ColourAttachments.empty())
		{
			...
			for (size_t i = 0; i < m_ColourAttachmentIds.size(); ++i)
			{
				...
				Internals::AttachColourTexture(
					m_ColourAttachmentIds[i],
					m_ColourAttachments[i],
					m_Properties.width,
					m_Properties.height,
					i);
			}
		...
		if (!m_ColourAttachments.empty())
		{
			glBindFramebuffer(GL_FRAMEBUFFER, m_RendererId);
			MGL_ASSERT(
				m_ColourAttachments.size() < MAX_COLOUR_ATTACHMENT_COUNT, 
				"Cannot have more than " + std::to_string(MAX_COLOUR_ATTACHMENT_COUNT) + " colour attachments.");
			MGL_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Error: Framebuffer is not complete.");
