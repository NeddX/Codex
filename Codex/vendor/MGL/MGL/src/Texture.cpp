...
namespace mgl
{
	Texture::Texture(const char* filePath, TextureProperties properties) : m_FilePath(filePath)
	{
		m_RendererId = 0;
		m_Buffer = nullptr;
		m_Width = 0;
		...
		*/
		...
		GL_Call(glTexImage2D(GL_TEXTURE_2D, 0, (GLint)properties.format, m_Width, m_Height, 0, GL_RGBA, GL_UNSIGNED_BYTE, m_Buffer));
		Bind();
	}
