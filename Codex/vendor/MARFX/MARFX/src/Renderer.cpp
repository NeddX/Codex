...
namespace mfx
{
	...
	void Renderer::RenderCall(const VertexArray* vertexArray, const IndexBuffer* indexBuffer, const Shader* shader) const
	{
		vertexArray->Bind();
		indexBuffer->Bind();
		shader->Bind();
	...
	void Renderer::RenderCall(const VertexArray* vertexArray, const Shader* shader, uint32_t count, uint32_t offset) const
	{
		vertexArray->Bind();
		shader->Bind();
